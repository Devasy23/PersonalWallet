...
def display_transactions():
    ...
    if response.status_code == 200:
        ...
        for transaction in transactions:
            st.write(f"Date: {transaction['date']}, Type: {transaction['type']}, Amount: {transaction['amount']}")
    else:
...
def add_transaction():
    with st.form("add_transaction_form"):
        ...
        transaction_type = st.selectbox("Type", ["Expense", "Income", "Debt Owed", "Debt Owing"])
        amount = st.number_input("Amount", min_value=0.01)
        add_button = st.form_submit_button("Add Transaction")

        if add_button:
            headers = {"Authorization": f"Bearer {st.session_state.auth_token}"}
            transaction_data = {"date": date, "type": transaction_type, "amount": amount}
            response = requests.post(TRANSACTIONS_URL, json=transaction_data, headers=headers)
...
def display_debts_and_stocks():
    ...
    if stocks_response.status_code == 200:
        ...
        for stock in stocks:
            st.write(f"Stock: {stock['name']}, Quantity: {stock['quantity']}, Price: {stock['price']}")
    else:
...
def main():
    ...
    if st.session_state.logged_in:
        ...
        page = st.sidebar.selectbox("Select Page", ["Transactions", "Add Transaction", "Debts and Stocks"])
