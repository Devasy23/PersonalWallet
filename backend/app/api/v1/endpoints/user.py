...
async def register(user: user_schema.UserCreate, db: AsyncIOMotorClient = Depends(get_db_client)):
    user_dict = user.dict()
...
@router.post("/login")
async def login(user: user_schema.UserLogin, db: AsyncIOMotorClient = Depends(get_db_client)):
